(function(root,factory){
  ...
  const REF_D=22.0,REF_CENTER_X=41.0,REF_INNER_R=11.0,REF_WALL=2.5,REF_HOLE_Y=18.0,REF_HOLE_D=5.0,REF_PLATE_X=7.0;
  const REF_BLEND_START=9.0,REF_BLEND_END=30.0,REF_ROOT=[28.0385186031843,6.5];
  ...
  const clamp=(n,a,b)=>Math.max(a,Math.min(b,n)),num=(v,f)=>Number.isFinite(Number(v))?Number(v):f,mod=a=>((a%(2*Math.PI))+2*Math.PI)%(2*Math.PI),smooth=t=>t*t*(3-2*t);
  ...
  function profile(options={}){const p=normalize(options),ch=p.arcChord/Math.max(1,p.scale);let cur=[PATH[0][1],PATH[0][2]],refs=[[cur[0],cur[1]]];for(let i=1;i<PATH.length;i++){const q=PATH[i];if(q[0]==='L'){cur=[q[1],q[2]];refs.push(cur)}else{const mid=[q[1],q[2]],end=[q[3],q[4]];for(const a of arcPoints(cur,mid,end,ch))refs.push(a);cur=end}}const clean=[];for(const q of refs){const t=transformPoint(q[0],q[1],p),a=clean.at(-1);if(!a||Math.hypot(t[0]-a[0],t[1]-a[1])>1e-8)clean.push(t)}return{params:p,points:clean}}
  function resolve(options={}){const pr=profile(options),p=pr.params,xs=pr.points.map(q=>q[0]),ys=pr.points.map(q=>q[1]);return{...pr,holeY:REF_HOLE_Y,holeSpacing:36,plateThickness:REF_PLATE_X,ringCenterX:p.L,innerDiameter:p.clipD,outerDiameter:p.clipD+2*p.wall,wall:p.wall,stemWidth:13,flangeWidth:59,filletR:2,lipRadii:[2.42127091581242*(p.wall/REF_WALL),1.25*(p.wall/REF_WALL),4.92127091581242*(p.wall/REF_WALL)],bounds2d:{xmin:Math.min(...xs),xmax:Math.max(...xs),ymin:Math.min(...ys),ymax:Math.max(...ys)},overall:{x:Math.max(...xs)-Math.min(...xs),y:Math.max(...ys)-Math.min(...ys),z:p.width}}}
  ...
  function mesh(options={}){
    ...
    const yB=29.5,yGap=8.5,hc=R.holeY,N=Math.max(16,p.quality);
  ...
  function binaryStl(options={}){const M=mesh(options),count=M.faces.length,buffer=new ArrayBuffer(84+count*50),dv=new DataView(buffer),head='CR20KB Snap-Fit Holder decoupled-L v1.1';for(let i=0;i<head.length&&i<80;i++)dv.setUint8(i,head.charCodeAt(i));dv.setUint32(80,count,true);let off=84;for(const pts of M.faces){const n=normal(...pts);for(const x of[...n,...pts[0],...pts[1],...pts[2]]){dv.setFloat32(off,x,true);off+=4}dv.setUint16(off,0,true);off+=2}return{buffer,mesh:M,fileName:fileName(options)}}
  return{REF_D,REF_CENTER_X,REF_HOLE_Y,REF_HOLE_D,PATH,DEFAULTS,LIMITS,normalize,profile,resolve,triangulate,splitHoleEdges,mesh,fileName,binaryStl};
});